...
(function (root, factory) {
  ...
  /**
   * validateL2New(proposedNew, opts) -> {valid, sanitized?|reason}
   * (R3D, estensione additiva — le 5 funzioni sopra restano invariate)
   *
   * Valida la FORMA di un'entry l2_new (osservazione nuova proposta da M2,
   * mai un id: lo assegna l'applicazione, non qui). Stessi vocabolari e
   * stesso vincolo unengaged=>none di validateL2Update. Una nuova
   * osservazione proposta già come "confirmed" è, di fatto, una
   * transizione-verso-confirmed da zero: richiede lo stesso gate
   * chef-attribution di una transizione su un item esistente — mai una
   * scorciatoia per aggirarlo passando da l2_new invece che da l2_updates.
   */
  function validateL2New(proposedNew, opts) {
    opts = opts || {};

    if (!proposedNew || typeof proposedNew !== 'object') {
      return { valid: false, reason: 'proposedNew non e\' un oggetto valido' };
    }
    if (OPERATIONAL_STATES.indexOf(proposedNew.operational_state) === -1) {
      return { valid: false, reason: 'operational_state mancante o non ammesso: ' + proposedNew.operational_state };
    }
    if (DECISION_STATES.indexOf(proposedNew.decision_state) === -1) {
      return { valid: false, reason: 'decision_state mancante o non ammesso: ' + proposedNew.decision_state };
    }
    if (proposedNew.operational_state === 'unengaged' && proposedNew.decision_state !== 'none') {
      return { valid: false, reason: 'unengaged richiede decision_state="none" (vincolo PSL)' };
    }
    if (!proposedNew.content || typeof proposedNew.content !== 'object' || typeof proposedNew.content.text !== 'string' || !proposedNew.content.text.trim()) {
      return { valid: false, reason: 'content mancante o privo di testo valido' };
    }
    if (proposedNew.decision_state === 'confirmed' && opts.chefAttributedThisTurn !== true) {
      return { valid: false, reason: 'una nuova osservazione proposta come "confirmed" richiede un segnale chef-attributable esplicito in questo turno (stesso gate epistemico di una transizione)' };
    }

    var sanitized = { operationalState: proposedNew.operational_state, decisionState: proposedNew.decision_state, content: proposedNew.content };
    if (proposedNew.evidence !== undefined) sanitized.evidence = proposedNew.evidence;
    return { valid: true, sanitized: sanitized };
  }

  /**
   * validateL3Candidate(candidate, existingL2ById) -> {valid, sanitized?|reason}
   * (R3D, estensione additiva)
   *
   * Valida SOLO la forma minima di un candidato L3: origin_l2_item_id
   * presente e riferito a un L2 realmente esistente (mai un id inventato
   * dall'AI, stessa disciplina già applicata a l2_updates), e
   * distilled_content presente. NON crea mai un L3Item, NON consolida
   * nulla: il gate reale (evidenza L2 sufficiente + decisione confirmed
   * attribuibile allo chef) resta responsabilità di uno step successivo.
   */
  function validateL3Candidate(candidate, existingL2ById) {
    existingL2ById = existingL2ById || {};
    if (!candidate || typeof candidate !== 'object') {
      return { valid: false, reason: 'l3_candidate non e\' un oggetto valido' };
    }
    if (!candidate.origin_l2_item_id) {
      return { valid: false, reason: 'origin_l2_item_id mancante' };
    }
    if (!existingL2ById[candidate.origin_l2_item_id]) {
      return { valid: false, reason: 'origin_l2_item_id non corrisponde a nessun L2 esistente: ' + candidate.origin_l2_item_id };
    }
    if (candidate.distilled_content === undefined || candidate.distilled_content === null) {
      return { valid: false, reason: 'distilled_content mancante' };
    }
    return {
      valid: true,
      sanitized: {
        distilledContent: candidate.distilled_content,
        originL2ItemId: candidate.origin_l2_item_id,
        contextConditions: candidate.context_conditions != null ? candidate.context_conditions : null,
        knownLimits: candidate.known_limits != null ? candidate.known_limits : null,
      },
    };
  }
  ...
  return {
    ...
    validateL2New: validateL2New,
    validateL3Candidate: validateL3Candidate,
  };
});
